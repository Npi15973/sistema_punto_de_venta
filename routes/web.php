...
Route::group(['middleware' => ['auth', 'active']], function() {
	...
	/**	rutas de facturacion electrónica */
	Route::get('emisor/{ruc}', 'BillerController@getDataEmisorservice')->name('emisor');
	...
	Route::get('products/imagenes/{id}','ProductController@getProductImage');
	...
	Route::get('sales/sri/{id}','SaleController@EnviarFacturaSri')->name('sale.sri');
	...
	Route::get('/pass', 'FacturacionController@obtenerPasswordFirma');
	Route::get('/certificado', 'FacturacionController@obtenerCertificado');
	Route::post('/validarFechaCertificado', 'FacturacionController@validarFechaCertificado');
	Route::post('/leerDocumento', 'FacturacionController@LeerDocumento');
	Route::post('/guardarXmlFirmado', 'FacturacionController@guardarDocumentoFirmado');
	Route::post('/enviarDocumento', 'FacturacionController@validarComprobante');
	Route::post('/autorizar', 'FacturacionController@autorizarComprobante');
	Route::get('/test-sri', 'FacturacionController@PruebaLlamadaSRI');
	Route::get('pdf/{id}','FacturacionController@ObtenerRutaPdf');
	Route::post('/procesarComprobante', 'FacturacionController@procesarComprobante')->name('facturacion.procesar');


	Route::get('ride/{id}/{documento}','FacturacionController@GenerarRidePDFVisual');
	Route::get('xml/{id}/{documento}','FacturacionController@descargarXml');

	Route::get('mail/{id}/{documento}','FacturacionController@EnviarMailRidePDF');
	Route::get('emisor','FacturacionController@Emisor')->name('emisor.inicio');
	Route::get('validar','FacturacionController@validarEmisor');
	Route::get('emisor/edit/{id}','FacturacionController@editar')->name('emisor.edit');
	Route::post('emisor/update','FacturacionController@update')->name('emisor.update');
	Route::get('emisor/delete/{id}','FacturacionController@eliminarEmisor')->name('emisor.delete');
	Route::get('documentos','FacturacionController@index2')->name('documentos');
	Route::post('emisor/create','FacturacionController@CrearEmisor')->name('emisor.create');
	Route::get('liquidacion/create','PurchaseController@liquidacion')->name('liquidacion');
	Route::get('liquidacion/lims_product_search', 'PurchaseController@limsProductSearch')->name('liquidacion.search');
	Route::post('liquidacion/purchases/purchase-data', 'PurchaseController@liquidacionData');
	Route::get('nota/credito', 'DocumentosElectController@notaCreditoIndex')->name('nota.credito');
	Route::get('nota/credito/create', 'DocumentosElectController@notaCreditoCreate')->name('nota.credito.create');
	Route::get('nota/credito/productos/{id}', 'DocumentosElectController@getProduct')->name('nota.credito.productos');
	Route::get('nota/credito/lims_product_search', 'SaleController@limsProductSearch')->name('product_sale.search');
	Route::post('nota/credito/agregar', 'DocumentosElectController@guardarProductoDetalle')->name('nota.credito.add');
	Route::post('nota/credito/insert', 'DocumentosElectController@InsertarNotaDebito')->name('nota.credito.insert');
	Route::get('nota/credito/delete/{id}', 'DocumentosElectController@eliminarDocumento')->name('nota.credito.delete');
	Route::get('nota/credito/anular/{id}', 'DocumentosElectController@anular')->name('nota.credito.anular');
	Route::get('nota/xml/{id}/{documento}','FacturacionController@descargarXml');

	Route::get('nota/debito', 'NotaDebitoController@index')->name('nota.debito');
	Route::get('nota/debito/create', 'NotaDebitoController@create')->name('nota.debito.create');
	Route::post('nota/debito/agregar', 'NotaDebitoController@AgregarMotivoTabla')->name('nota.debito.add');
	Route::post('nota/debito/eliminar', 'NotaDebitoController@quitarMotivo')->name('nota.debito.delete');
	Route::post('nota/debito/insert', 'NotaDebitoController@GuardarDocumento')->name('nota.debito.insert');
	Route::get('nota/debito/delete/{id}', 'NotaDebitoController@eliminarDocumento')->name('nota.debito.delete');
	Route::get('nota/debito/anular/{id}', 'NotaDebitoController@anular')->name('nota.debito.anular');


	Route::get('retencion', 'RetencionController@index')->name('retencion');
	Route::get('retencion/create', 'RetencionController@create')->name('retencion.create');
	Route::post('retencion/agregar', 'RetencionController@agregarRetencionTabla')->name('retencion.agregar');
	Route::post('retencion/eliminar', 'RetencionController@quitarRetencion')->name('retencion.delete');
	Route::post('retencion/insert', 'RetencionController@store')->name('retencion.insert');
	Route::get('retencion/delete/{id}', 'RetencionController@eliminarDocumento')->name('retencion.delete');
	Route::get('retencion/anular/{id}', 'RetencionController@anular')->name('retencion.anular');

	Route::get('guia', 'GuiaController@index')->name('guia');
	Route::get('guia/create', 'GuiaController@create')->name('guia.create');
	Route::get('guia/productos/{id}', 'GuiaController@getProduct')->name('guia.productos');
	Route::post('guia/agregar', 'GuiaController@guardarProductoDetalle')->name('guia.add');
	Route::post('guia/eliminar', 'GuiaController@quitarProducto')->name('guia.delete');
	Route::post('guia/insert', 'GuiaController@store')->name('guia.insert');
	Route::get('guia/delete/{id}', 'GuiaController@eliminarDocumento')->name('guia.delete');
	Route::get('guia/anular/{id}', 'GuiaController@anular')->name('guia.anular');

	/**
	 * test
	 */
	Route::get('test-fact/{id}','SaleController@crearFacturaXml')->name('test.doc');
	
	Route::get('test-not/{id}','NotaDebitoController@generarNotaDebitoXml')->name('test.doc');
	Route::get('test-ride/{id}/{documento}','FacturacionController@EnviarMailRidePDF')->name('test.ride');	
	//Route::get('test-not/{id}', 'DocumentosElectController@generarNotaCreditoXml')->name('test.nota');
});
