...
use App\Librerias\Validadores;

class CustomerController extends Controller
{

    private $validadores;
    public function __construct(Validadores $validadores){
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('customers-add')){
            ...
            return view('customer.create', compact('lims_customer_group_all','tipo_documento'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function store(Request $request)
    {
        ...
        $documento = $lims_customer_data["tax_no"];
        if($lims_customer_data["tipo_identificacion"]=="05"){// cedula
            if(!$this->validadores->validarCedula($documento)){
                return redirect()->back()->with('not_permitted', 'Cédula incorrecta');
            } 
        }
        if($lims_customer_data["tipo_identificacion"]=="04"){// ruc
            if(!$this->validadores->validarRucPersonaNatural($documento)){
                return redirect()->back()->with('not_permitted', 'Ruc incorrecto');
            } 
            if(!$this->validadores->validarRucSociedadPrivada($documento)){
                return redirect()->back()->with('not_permitted', 'Ruc incorrecto');
            } 
            if(!$this->validadores->validarRucSociedadPublica($documento)){
                return redirect()->back()->with('not_permitted', 'Ruc incorrecto');
            } 
        }
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('customers-edit')){
            ...
            $lims_customer_group_all = CustomerGroup::where('is_active',true)->get();
            return view('customer.edit', compact('tipo_documento','lims_customer_data','lims_customer_group_all'));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $input = $request->all();
        $documento = $input["tax_no"];
        if($input["tipo_identificacion"]=="05"){// cedula
            if(!$this->validadores->validarCedula($documento)){
                return redirect()->back()->with('not_permitted', 'Cédula incorrecta');
            } 
        }
        if($input["tipo_identificacion"]=="04"){// ruc
            if(!$this->validadores->validarRucPersonaNatural($documento)){
                return redirect()->back()->with('not_permitted', 'Ruc incorrecto');
            } 
            if(!$this->validadores->validarRucSociedadPrivada($documento)){
