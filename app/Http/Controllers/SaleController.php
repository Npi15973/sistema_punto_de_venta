...
use App\Services\ProcesarDocumentoService;

class SaleController extends Controller
{
    private $comprobante;
    private $facturacion;
    private $procesarComprobanteService;
    public function __construct(Comprobante $comprobante, FacturacionElectronica $facturacion ,ProcesarDocumentoService $procesarComprobanteService){
        $this->comprobante=$comprobante;
        $this->facturacion=$facturacion;
        $this->procesarComprobanteService= $procesarComprobanteService;
    }
    /**
     * inicio 
     */
    public function index()
    {
        ...
        if($role->hasPermissionTo('sales-index')) {
            ...
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own'){
                $lims_sale_all = Sale::orderBy('id', 'desc')->where('ambiente',$ambiente)->where('user_id', Auth::id())->get();
            } else{
                $lims_sale_all = Sale::where('ambiente',$ambiente)->orderBy('id', 'desc')->get();
                $lims_gift_card_list = GiftCard::where("is_active", true)->get();
                $lims_pos_setting_data = PosSetting::latest()->first();
                $lims_account_list = Account::where('is_active', true)->get();
            }

            return view('sale.index',compact('lims_sale_all', 'lims_gift_card_list',
             'lims_pos_setting_data', 'lims_account_list', 'all_permission'));
        }
        else
    ...
    public function saleData(Request $request)
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own'){
            $totalData = Sale::where('user_id', Auth::id())->where('ambiente',$ambiente)->count();  
        }else{
            $totalData = Sale::where('ambiente',$ambiente)->count();
        ...
        if(!empty($sales))
        {
            foreach ($sales as $key=>$sale)
            {
                ...
                $nestedData['key'] = $key;
                $nestedData['date'] = date($general_setting->date_format, strtotime($sale->fecha_emision));
                ...
                $nestedData['clave_acceso'] = $sale->clave_acceso;
                $nestedData['fecha_autorizacion'] = $sale->fecha_autorizacion;
                $css ="badge badge-warning";
                if($sale->estado_fact_sri=="autorizado"){
                    $css ="badge badge-success";
                }
                $botonEnviarSri="";
                $botonEnviarMailCliente="";
                
                if($sale->estado_fact_sri=="creado"){
                    $botonEnviarSri = '<li>
                                        <form action="/procesarComprobante" method="POST">
                                            <input type="hidden" name="documentId" value="'.$sale->id.'">
                                            <input type="hidden" name="documentType" value="01">
                                            <input type="hidden" name="_token" value="'.csrf_token() .'" />
                                            <button type="submit"  class="btn btn-link"><i class="fa fa-paper-plane"></i>Enviar SRI</button>
                                        </form>     
                                    </li>';
                }else if($sale->estado_fact_sri=="autorizado"){
                    $botonEnviarMailCliente='<li>
                    <button type="button" onclick="enviarRideCliente('.$sale->id.',\'01\')" class="btn btn-link"><i class="fa fa-paper-plane"></i>Renviar Mail</button>
                </li>';
                }
                
                $nestedData['factura'] = '<div class="'.$css.'">'.$sale->estado_fact_sri.'</div>';
                $nestedData['options'] = '<div class="btn-group">
                            <button type="button" class="btn btn-default dropdown-toggle btn-sm " data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span class="fa fa-list"></span>
                            ...
                            <ul class="dropdown-menu edit-options dropdown-menu-right dropdown-default" user="menu">'.$botonEnviarSri.''.$botonEnviarMailCliente.'<li>
                                <button type="button" onclick="GenerarPdf('.$sale->id.',\'01\')" class="btn btn-link"><i class="fa fa-eye"></i>Generar Descargar PDF</button>
                                </li><li>
                                <button type="button" onclick="DescargarXml('.$sale->id.',\'01\')" class="btn btn-link"><i class="fa fa-download"></i>Descargar Xml</button>
                                </li>';
                if(in_array("sales-edit", $request['all_permission'])){
                    if($sale->sale_status != 3){
                    if($sale->estado_fact_sri=="creado"){
                        $nestedData['options'] .= '<li>
                            <a href="'.route('sales.edit', $sale).'" class="btn btn-link"><i class="fa fa-edit"></i> '.trans('file.edit').'</a>
                            </li>';
                    }
    ...
    public function create()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            $tipo_documento= TipoDocumento::all();
            //return $lims_biller_list;
            return view('sale.create',compact('lims_customer_list', 'lims_warehouse_list',
            'lims_biller_list', 'lims_pos_setting_data', 'lims_tax_list','formas_pago','tipo_documento'));
        }
        else
            return redirect()->back()->with('not_permitted', '¡Lo siento! No tienes permiso para acceder a este módulo');
    }
    ...
    public function store(Request $request)
    {
        $data = $request->all();
        $message = "";
        $cliente = Customer::where('id',$data['customer_id'])->first();
        $emisor= Emisor::where('is_active',1)->first();
        $ambiente = $emisor->ambiente;
        $tpoEmi = Biller::where('id',$data["biller_id"])->first();
        
        $data["ambiente"]= $emisor->ambiente;
        $data["tipo_documento"]=$cliente->tipo_identificacion;
        $data['estado_fact_sri'] = "creado";
        $data['user_id'] = Auth::id();
        $wareh= Warehouse::where('id',$data['warehouse_id'])->first();
        if($data['pos']){
            $data['fecha_emision'] = date("Y-m-d");
            $data['reference_no'] = $wareh->cod_ref .'-POS-' . date("Ymd") . '-'. date("his");
            ...
            $data['reference_no'] = $wareh->cod_ref ."-". date("Ymd") . '-'. date("his");
        }
        ...
        $lastSale = Sale::where('ambiente',$ambiente)->latest()->first();
        if($lastSale){
            $secuencial = $lastSale->secuencial+1;
            
        }else{
            
            $secuencial = $tpoEmi->secuencial_factura;
        }
        ...
        $data["numero_documento"] = $wareh->codigo."-".$tpoEmi->codigo."-".$secuencial_final;
        ...
        if($data['sale_status'] == 3)
            $message = 'Venta agregada correctamente al borrador';
        else
        ...
        if($mail_data['email'] && $data['sale_status'] == 1) {
            ...
            try {
                ...
                {
                    $message->to( $mail_data['email'] )->subject( 'Detalles de venta' );
                });
            }
            catch(\Exception $e){
                $message = ' Venta creada con éxito. Configura tu <a href="setting/mail_setting">mail setting</a> enviar correo.';
            }*/
        }
        ...
        if($data['payment_status'] == 3 || $data['payment_status'] == 4 || ($data['payment_status'] == 2 && $data['pos'] && $data['paid_amount'] > 0)) {
            ...
            if($data['pos']){
                $data['payment_reference'] = $wareh->cod_ref."-POS-".date("Ymd").'-'.date("his");
            }else{
                $data['payment_reference'] = $wareh->cod_ref."-".date("Ymd").'-'.date("his");
            }
            $metodo_pago= FormaDePago::where('codigo',$data["forma_pago"])->first();
            $lims_payment_data->payment_reference = $data['payment_reference'];
            $lims_payment_data->amount = $data['paid_amount'];
            $lims_payment_data->change = $data['paying_amount'] - $data['paid_amount'];
            $lims_payment_data->paying_method = $metodo_pago->descripcion;
            $lims_payment_data->payment_note = $data['payment_note'];
            
            $lims_payment_data->forma_pago = $data["forma_pago"];
            $lims_payment_data->save();

        }
        
       $this->crearFacturaXml($newSale->id);
        
       $dataFacturacion=array();
       $dataFacturacion["documentId"]= $newSale->id;
       $dataFacturacion["documentType"]= "01";
        $mensajeFacturacion = $this->procesarComprobanteService->procesarComprobante($dataFacturacion);
        if($data['pos']){
            
            return redirect('pos')->with('message', $mensajeFacturacion);
        }
        else{

            return redirect('sales')->with('message', $mensajeFacturacion);
        }
    }


    public function crearFacturaXml($id , $esEdit =false){

        $data_fact_electronica = Sale::where('id',$id)->first();
        $data_warehause= Warehouse::where('id',$data_fact_electronica->warehouse_id)->first();
        $emisor= Emisor::where('is_active',1)->first();
        $data_fact_electronica['emisor'] = $emisor;
        $data_fact_electronica['dirEstablecimiento'] = $data_warehause->address;
        $data_fact_electronica['dirMatriz']=$emisor->direccion_matriz;
        $data_fact_electronica['codDoc']="01";// codigo de factura
        $data_fact_electronica['fechaEmision']=$data_fact_electronica->fecha_emision; 
        $data_fact_electronica['estab']=$data_warehause->codigo;
        $ptoEmision = Biller::where('warehouse_id',$data_warehause->id)->first();
        $data_fact_electronica['ptoEmi']=$ptoEmision->codigo;
        $data_fact_electronica['codigo_numerico']=$data_warehause->codigo_numerico;
        $secuencial_inicial = $data_fact_electronica->secuencial;
        $num_sec_ide =9;
        $num_sec = strlen($secuencial_inicial);
        $falta = $num_sec_ide - $num_sec;
        $alterno="0";
        ...
        $secuencial_final = $alterno.$secuencial_inicial;
        //$numero_docuemento;
        $data_fact_electronica['secuencial']=$secuencial_final;
        $data_fact_electronica['cliente']=Customer::where('id',$data_fact_electronica->customer_id)->first();
         $detalles = DB::table('products')
        ->join('product_sales', 'products.id', '=', 'product_sales.product_id')
        ->where([
            ['products.is_active', true],
            ['product_sales.sale_id', $id]
        ])->select('product_sales.*','products.name','products.code')->get();
        

        //return count($detalles);
        if($data_fact_electronica["shipping_cost"]>0){
            $det = new Product_Sale();
                $det->id=0;
                $det->sale_id=0;
                $det->product_id='EN01';
                $det->qty=1;
                $det->sale_unit_id=0;
                $det->net_unit_price=$data_fact_electronica["shipping_cost"];
                $det->discount=0;
                $det->tax_rate=0;
                $det->tax=0;
                $det->total=$data_fact_electronica["shipping_cost"];
                $det->created_at= date('Y-m-d');
                $det->updated_at=date('Y-m-d');
                $det->company_id=0;
                $det->name='Costo Envío';
                $det->code='EN01'; 
            $detalles[count($detalles)] = $det;
        }
        $data_fact_electronica['detalle'] =$detalles;
        //return $data_fact_electronica;

         $factura = $this->comprobante->generarFactura($data_fact_electronica);
         //return json_encode($factura);
         $claveAcceso= $this->getClaveAcceso($factura);
         $xml = $this->facturacion->generarFacturaXml($factura);
         if($esEdit){
            File::delete(public_path('facturacion/facturacionphp/comprobantes/no_firmados/'.$claveAcceso.'.xml'), $xml);
         }
    ...
    public function EnviarFacturaSri($id){
        ...
        //return Storage::download('facturas/'.$claveAcceso.'.xml');
    }
    ...
    public function sendMail(Request $request)
    {
        ...
        if($lims_customer_data->email) {
            ...
            /*try{
                ...
                $message = 'El correo enviado con éxito';
            }
            catch(\Exception $e){
                $message = 'Por favor configure su <a href="setting/mail_setting">mail</a> para enviar correo.';
            }*/
        }
        else
    ...
    public function getProductByFilter($category_id, $brand_id)
    {
        ...
        if(($category_id != 0) && ($brand_id != 0)){
            $lims_product_list = DB::table('products')
                                ...
                                ->where([
                                    ...
                                    ['products.is_active', true],
                                    ['brand_id', $brand_id]
                                ])->distinct()->select('products.name', 'products.code', 'product_images.imagen_base64 as image')->get();
        }
        ...
        foreach ($lims_product_list as $key => $product) {
            ...
            //$images = ProductImage::where("products_id",$product->id)->first();// explode(",", $product->image);
    ...
    public function getFeatured()
    {
        ...
        foreach ($lims_product_list as $key => $product) {
            ...
            $data['code'][$key] = $product->code;
            $images = ProductImage::where("products_id",$product->id)->first();//explode(",", $product->image);
            $data['image'][$key] = $images->imagen_base64;
        }
        return $data;
    }
    ...
    public function saleByCsv()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            return view('sale.import',compact('lims_customer_list', 'lims_warehouse_list',
            'lims_biller_list', 'lims_tax_list',));
        }
        else
    ...
    public function createSale($id)
    {
        ...
        if($role->hasPermissionTo('sales-edit')){
            ...
            return view('sale.create_sale',compact('lims_biller_list', 'lims_customer_list',
            'lims_warehouse_list', 'lims_tax_list', 'lims_sale_data','lims_product_sale_data',
            'lims_pos_setting_data', 'lims_brand_list', 'lims_category_list', 'lims_coupon_list',
            'lims_product_list', 'product_number', 'lims_customer_group_all',));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function edit($id)
    {
        ...
        if($role->hasPermissionTo('sales-edit')){
            ...
            return view('sale.edit',compact('lims_customer_list',
             'lims_warehouse_list', 'lims_biller_list', 'lims_tax_list', 
             'lims_sale_data','lims_product_sale_data',));
        }
        else
            return redirect()->back()->with('not_permitted', 'Sorry! You are not allowed to access this module');
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $this->crearFacturaXml($id,true);
        //$this->crearFacturaXml($id);
        
       $dataFacturacion=array();
       $dataFacturacion["documentId"]= $id;
       $dataFacturacion["documentType"]= "01";
        $mensajeFacturacion = $this->procesarComprobanteService->procesarComprobante($dataFacturacion);
    ...
    public function updatePayment(Request $request)
    {
        ...
        elseif ($data['edit_paid_by_id'] == 3){
            ...
            if($lims_payment_data->paying_method == 'Credit Card'){
                ...
                $lims_payment_with_credit_card_data->charge_id = "999999999";
                ...
                if(!$lims_payment_with_credit_card_data) {
                    ...
                    $data['customer_stripe_id'] = "999999999";
                }
                $data['customer_id'] = $lims_sale_data->customer_id;
                $data['charge_id'] = $charge->id;
                PaymentWithCreditCard::create($data);
    ...
    public function posSale()
    {
        ...
        if($role->hasPermissionTo('sales-add')){
            ...
            foreach ($lims_product_list as $key => $product) {
                ...
                $images = ProductImage::where("products_id",$product->id)->first();
                $product->base_image = $images->imagen_base64;
            }
            $product_number = count($lims_product_list);
            $lims_pos_setting_data = PosSetting::latest()->first();
            $lims_brand_list = Brand::where('is_active',true)->get();
            ...
            $formas_pago= FormaDePago::all();
            $tipo_documento = TipoDocumento::all();
            return view('sale.pos', compact('tipo_documento','all_permission', 'lims_customer_list',
            'lims_customer_group_all', 'lims_warehouse_list', 'lims_product_list',
             'product_number', 'lims_tax_list', 'lims_biller_list', 'lims_pos_setting_data',
             'lims_brand_list', 'lims_category_list', 'recent_sale', 'recent_draft',
             'lims_coupon_list', 'flag','formas_pago'));
        }
        else
            return redirect()->back()->with('not_permitted', '¡Lo siento! No tienes permiso para acceder a este módulo');
    }
