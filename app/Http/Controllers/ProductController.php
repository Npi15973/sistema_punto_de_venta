...
class ProductController extends Controller
{
    ...
    public function getProductImage($id){
        $imagenes = ProductImage::where('products_id',$id)->get();
        return $imagenes;
    }
        ...
        if(!empty($products))
        {
          //return $products;
            foreach ($products as $key=>$product)
            {
                ...
                $imagenesProducto = ProductImage::where('products_id',$product->id)->get();
                $srcIm = '<img src="#" height="80" width="80">';
                if(count($imagenesProducto)>0)
                {
                    $srcIm = '<img src="'.$imagenesProducto[0]->imagen_base64.'" height="80" width="80">';
                }
                $nestedData['image'] = $srcIm;
                ...
                if(in_array("products-edit", $request['all_permission']))
                    $nestedData['options'] .= '<li>
                            <a href="'.route('products.edit',  $product).'" class="btn btn-link"><i class="fa fa-edit"></i> '.trans('file.edit').'</a>
                        </li>';
    ...
    public function store(Request $request)
    {
      try {
        ...
        $producto_nuevo =  Product::create($data);
        ...
        if($images) {
          ...
          $imagen_base64="";
            ...
            foreach ($images as $key => $image) {
                ...
                $images_base64[$contador]['products_id'] = $producto_nuevo->id;
                $images_base64[$contador]['imagen_base64'] = $imagen_base64;

                $contador++;

            }

            ProductImage::insert($images_base64);
        ...
        if($images) {
            ...
            $imagen_base64="";
              ...
              foreach ($images as $key => $image) {
                  ...
                  $image->resize(800, 500);
                  $imagen_base64 = $image->encode('data-url');
                  $images_base64[$contador]['products_id'] = $request->input('id');
                  $images_base64[$contador]['imagen_base64'] = $imagen_base64;
